...
export const setAccessTokenCookie = (token: string) => {
  ...
  const defaultExpiration = new Date(Date.now() + 24 * 60 * 60 * 1000); // Default expiration in 1 day
  const expiresOption = expirationTime ? { expires: new Date(expirationTime) } : { expires: defaultExpiration };
